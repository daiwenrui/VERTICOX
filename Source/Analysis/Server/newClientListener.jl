...
#  println("data listener has been closed");
  ...
  writecsv("C:/VERTICOX/Source/Analysis/Server/Surv.txt",result["Surv"]);
  writecsv("C:/VERTICOX/Source/Analysis/Server/AUC.txt",result["AUC"]);
...
#        println("Update calculation at iter: ", result["iter"]+1);
        println("msg is updateCalculation");
...
function asyncCheckMessageFromDataListener(closeServerFlag, msg, socks_list, TIMEOUT, MSG_CODEBOOK, DB)
  msgminus1=1;
#  println("asyncCheckMessage: msg is ", msg);
  if !isempty(msg)
    ...
    elseif msg["tag"] == MSG_CODEBOOK["updateCalculation_Done"]
      println("Finished AUC computation");
      msgminus1 = -1;
...
#  println("tmp_sock is open? ", isSockOpen, "tmp_sock_data is open?", isSockOpen);
  if isSockOpen
#    println("addOneClientIntoNewSockets_list");
